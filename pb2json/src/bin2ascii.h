/*
 * Copyleft by jimmy.gj
 * Base on Pavel Shramov <shramov@mexmat.net>
 *
 */
...
inline std::string hex2bin(const std::string &s)
{
	...
	if (s.size() % 2)
    {
        std::cerr << "Odd hex data size in hex2bin, then return empty string" << std::endl;
        return r;
    }
	...
	for (size_t i = 0; i < s.size(); i += 2)
    {
		...
		if (0x80 & (hi | lo))
        {
            std::cerr << "Invalid hex data: " << 
                s.substr(i, 6) << ", then return empty string" << std::endl;
            r.clear();
            return r;
        }
		r.push_back((hi << 4) | lo);
...
inline std::string b64_decode(const std::string &s)
{
	...
	if (s.size() % 4)
    {
        std::cerr << "Invalid base64 data size, return empty" << std::endl;
        return r;
    }
	size_t pad = 0;
	if (s[s.size() - 1] == '=') pad++;
	...
	for (size_t i = 0; i < s.size(); i += 4)
    {
		...
		if (0x80 & (n0 | n1 | n2 | n3))
        {
            std::cerr << "Invalid hex data: " << s.substr(i, 4) 
                << ", return empty"
                << std::endl;
            r.clear();
            return r;
